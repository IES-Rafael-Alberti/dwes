#+TITLE: Blog con Laravel 10 y sail
#+AUTHOR:
#+EMAIL:
#+DATE:
...
* Blog con Laravel 10 sail
El código del proyecto se puede encontrar [[https://github.com/IES-Rafael-Alberti/blog-laravel10.git][aquí]].


** Crear proyecto
#+begin_src bash
curl -s "https://laravel.build/blog-laravel10?with=pgsql,redis,selenium" | bash
cd blog-laravel10
sail up
#+end_src
...
hacer «seguro», es crear los tests, pulsamos =Alt+Insert= y en el
menú, una de las opciones, es Test...).

Ya existe una opción para crear vistas con =artisan= y podemos crearla así:
#+begin_src bash
...
* TODO Subida de imagenes :noexport:
...
*** Mostrar avatar en vista de artículo completo
También podemos mostrar el avatar en la vista =show.blade.php=. Para
ello podemos reciclar el código usado anteriormente para visualizar el
avatar, con un ligero cambio. Hasta ahora usábamos
~Auth::user()->avatar~ para recuperar la imagen, pero ahora
necesitamos la imagen del creador del artículo, no la del usuario
autenticado. Cambiamos lo anterior por
~$article->user->avatar~. Buscamos la imagen de usuario sin avatar y
...
Pero no funciona... el avatar está a =null=, no lo estamos
recibiendo. Necesitamos cambiar la función ~show()~ en
ArticleController a:
#+begin_src php
  <?php
public function show(Article $article): Renderable
...
~$article~ a través de la relación con el usuario.
...
*** PHPStorm
[[https://github.com/NiclasvanEyk/jetbrains-laravel-make-integration/blob/main/docs/setup_with_sail.md][Laravel Make Integration]], plugin para usar artisan make:* desde un
...
- Por uno mismo
  + [[https://laravel.com/docs/9.x/deployment][Con Nginx]]
- Con servicio
  + [[https://bootcamp.laravel.com/deploying][Laravel Forge (cuenta de prueba)]]
  + [[https://vapor.laravel.com/][Laravel Vapor (serverless)]]
...
Información de cómo [[https://github.com/barryvdh/laravel-debugbar][usar Debugbar...]]
