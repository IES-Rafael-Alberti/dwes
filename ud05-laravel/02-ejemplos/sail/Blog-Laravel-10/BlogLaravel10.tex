...
\date{}
\title{Blog con Laravel 10 y sail}
\hypersetup{
 pdfauthor={},
 pdftitle={Blog con Laravel 10 y sail},
 pdfkeywords={},
 pdfsubject={},
...
\section{Blog con Laravel 10 sail}
\label{sec:org044b4cb}
...
\label{sec:org8e0110b}
\begin{minted}[]{bash}
curl -s "https://laravel.build/blog-laravel10?with=pgsql&php=81" | bash
cd blog-laravel10
sail up
\end{minted}

\subsection{Modelo de datos}
\label{sec:org02a7f68}
\subsubsection{Modelos y sus migraciones}
\label{sec:org5d473d0}

\begin{itemize}
\item Creamos el modelo \emph{Category} y su migración añadiendo \texttt{-m}
\label{sec:org685e3d7}
\begin{minted}[]{bash}
...
Vamos a trabajar con Laravel Blade (Alpine.js) para las vistas, es la
forma más sencilla. No es la más potente ni la única, Laravel se puede
integrar fácilmente con Vue y con React, y con un poco más de trabajo
con Mithril.js...
...
hacer «seguro», es crear los tests, pulsamos \texttt{Alt+Insert} y en el
menú, una de las opciones, es Test...).

Ya existe una opción para crear vistas con \texttt{artisan} y podemos crearla así:
\begin{minted}[]{bash}
sail artisan make:view articles.index
\end{minted}
...
de "articles" \texttt{:active="request()->routeIs('articles.*')} \\[0pt]
\texttt{\{\{ \_\_('Artículos') \}\}}.
...
Vemos que el contenido realmente está en la etiqueta \texttt{<div class="-my-8 divide-y-2} \\[0pt]
...
vamos a devolver uno del tipo \\[0pt]
...
Instalación temporal para Laravel 10 de versión anterior de Pest a la
2, que requiere PHP 8.2 y nuestro proyecto tiene PHP 8.1.2.
\begin{minted}[]{bash}
sail composer require phpunit/phpunit:^9.6 nunomaduro/collision:^6.1 \
     pestphp/pest --dev --with-all-dependencies
\end{minted}
...
Este paso, ahora mismo ya no es necesario:
...
\subsubsection{Mostrar avatar en vista de artículo completo}
\label{sec:org8c3669c}
También podemos mostrar el avatar en la vista \texttt{show.blade.php}. Para
ello podemos reciclar el código usado anteriormente para visualizar el
avatar, con un ligero cambio. Hasta ahora usábamos
\texttt{Auth::user()->avatar} para recuperar la imagen, pero ahora
necesitamos la imagen del creador del artículo, no la del usuario
autenticado. Cambiamos lo anterior por
\texttt{\$article->user->avatar}. Buscamos la imagen de usuario sin avatar y
añadimos el código encima (si alguien encuentra una posición mejor,
que avise...) y quedaría así.
\begin{minted}[]{php}
<div class="rounded-lg overflow-hidden">
  <h1 class="text-3xl">{{ $article->title }}</h1>
</div>
...
Pero no funciona... el avatar está a \texttt{null}, no lo estamos
recibiendo. Necesitamos cambiar la función \texttt{show()} en
ArticleController a:
\begin{minted}[]{php}
  <?php
public function show(Article $article): Renderable
...
Hemos añadido un tercer parámetro a la lista de datos que metemos en
\texttt{\$article} a través de la relación con el usuario.
...
\section{Anexo I}
\label{sec:org1c095b1}
\subsection{Herramientas, librerías etc.}
\label{sec:org2481fe6}
\subsubsection{Cliente}
\label{sec:orgef1c2e7}
\begin{itemize}
...
\label{sec:org9b44064}
\subsubsection{Instalación}
\label{sec:org563c5e5}
\begin{minted}[]{bash}
sail composer require barryvdh/laravel-debugbar --dev
\end{minted}

\subsection{Uso}
\label{sec:org802f195}

Información de cómo \href{https://github.com/barryvdh/laravel-debugbar}{usar Debugbar...}
