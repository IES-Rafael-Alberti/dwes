...
- Script SQL completo `pedidos_hibridos.sql` con el DDL, el trigger de validación, inserciones de prueba y las consultas con `EXPLAIN ANALYZE`.
- Documento `comparativa.md` con la comparación detallada de este modelo frente a MongoDB (con dibujo de esquema Mongo).
...
| Aspecto | |
|---------|-|
| Uso de IA permitido | Sí, para generar esqueletos del trigger PL/pgSQL o del DDL de creación de tablas. |
| Declaración obligatoria | Sí. |
| Herramientas permitidas | ChatGPT, Claude, Gemini, GitHub Copilot. |
| Qué NO está permitido | Generar las conclusiones de la comparativa con MongoDB ni el análisis de performance del explain de forma automatizada sin comprensión real del funcionamiento de los índices. |
